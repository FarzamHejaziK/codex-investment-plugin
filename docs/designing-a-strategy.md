...
Every strategy is a single markdown file in `<workspace>/strategies/`. The daily workflow reads each file with `status: active`, follows its rules, and proposes today's orders. In trading-with-confirmation mode, it may submit only the exact proposed order batch the user confirms.

This doc explains the anatomy of a strategy file so you can write your own or modify the examples.

For an interactive builder, use the new-strategy workflow. It asks questions and generates a paused file in your workspace.
...
These keys control how the daily workflow treats the strategy.

| Field | Required? | Values | What it does |
|---|---|---|---|
| `name` | Yes | lowercase-hyphenated | Used for logging and tagging journal entries |
| `status` | Yes | `paused` / `active` / `archived` | The daily workflow only processes `active` strategies |
...
| `auto_research` | No | `daily` | If set, the daily workflow does per-name news scans and appends to "Live research notes" |
| `review_cadence` | No | `quarterly` | If set, the daily workflow on the 1st of Feb/May/Aug/Nov produces extended trade-log summary |

## The body sections

The body is markdown. Write it however you want, but the daily workflow looks for certain section names. The required ones are:
...
The list of tickers this strategy is allowed to trade. **Be explicit and exhaustive** — the daily workflow enforces "no instruments outside this list."
