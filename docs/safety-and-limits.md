...
This doc covers what the workspace will and will not do, why the defaults are conservative, and what changes when you enable trading-with-confirmation.

## What this tool is

A Codex workspace for organizing investment strategies. The daily workflow reads strategy files, pulls Alpaca portfolio data, writes a daily memo, and proposes specific orders based on rules you wrote.
...
2. The daily workflow proposes orders and writes the memo.
...
1. The daily workflow runs the same analysis and writes the same memo.
...
- **Bad strategies can lose money.** The workspace follows your rules. It does not prove those rules are profitable.
...
- Setup, help, and strategy-builder workflows never place trades.
- The daily workflow is the only workflow that may place orders, and only in trading-with-confirmation mode.
